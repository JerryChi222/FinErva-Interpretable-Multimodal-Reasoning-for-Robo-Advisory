...
def get_acc_with_contion(res_pd, key, values):
    ...
    if len(total_pd) == 0:
        # 没有样本时返回 0.00（也可以返回 "N/A" 或者 None）
...
def get_scores(result_data, rationale_data, results_reference, data_file):
    ...
    res_pd = sqa_pd[sqa_pd['split'] == 'test'].head(num) # 只取num个测试数据
    # update data
    # print("All test-set indices:", res_pd.index.tolist())
    for index, row in res_pd.iterrows():
        # print(f"First index: {index!r}  (type: {type(index)})")
        res_pd.loc[index, 'no_context'] = True if (not row['lecture'] and not row['image']) else False
        res_pd.loc[index, 'has_text'] = True if row['lecture'] else False
        res_pd.loc[index, 'has_image'] = True if row['image'] else False
        res_pd.loc[index, 'has_text_image'] = True if (row['lecture'] and row['image']) else False
    ...
    ## Similarity
    model = SentenceTransformer("models/all-MiniLM-L6-v2", device="cuda") # 用于计算输出指标的模型，计算思维链相似度
