...
class JointEncoder(T5Stack):
    ...
    def forward(
        ...
        if image_ids is None:
            # batch_size 与 hidden_states 一致
            batch_size = hidden_states.size(0)
            # 从 self.image_dense 推断输入维度
            in_features = self.image_dense.weight.size(1)
            # 全 0 tensor，shape=(batch_size, in_features)
            image_ids = torch.zeros(batch_size, in_features, device=hidden_states.device)
