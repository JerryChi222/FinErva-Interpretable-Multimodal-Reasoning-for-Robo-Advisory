...
def load_data_img(args):
    problems = json.load(open(os.path.join(args.data_root, 'data/problems.json')))
    pid_splits = json.load(open(os.path.join(args.data_root, 'data/pid_splits.json')))
    captions = json.load(open(os.path.join(args.data_root, 'instruct_captions.json')))["captions"]
    name_maps = json.load(open(os.path.join(args.data_root, 'name_map.json')))
    ...
    if args.img_type == "resnet":
        ...
        if args.use_fin == "pact":
            image_features = torch.load("vision_features-fin-pact/vit.pth")
            print("loading Finerva-Pact features......")
        if args.use_fin == "price":
            image_features = torch.load("vision_features-fin-price/vit.pth")
            print("loading Finerva-Price features......")
    else:
        image_features = np.load('vision_features/detr.npy')
