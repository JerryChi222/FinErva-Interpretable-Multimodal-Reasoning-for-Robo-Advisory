...
def parse_args():
    ...
    parser.add_argument('--use_fin', type=str, default='pact', choices=['pact', 'price'], help='use a valid dataset')

    parser.add_argument('--use_generate', action='store_true', default=True, help='only for baseline to improve inference speed')
    ...
    parser.add_argument('--prompt_format', type=str, default='QCM-A', help='prompt format template',
                        choices=['QCM-A', 'QCM-E', 'QCM-LE', 'QCMG-EA', 'QCM-LEA', 'QCM-ALE', 'QC-AL', 'QC-A', 'QC-EA', 'QCMG-EA', 'QCA-A', 'QCMG-A'])
...
def T5Trainer(
    ...
    if args.evaluate_dir is not None:
        ...
        save_dir = f"{args.output_dir}/{args.use_fin}_{args.user_msg}_{model_name}_{args.img_type}_{args.prompt_format}_lr{args.lr}_bs{args.bs * gpu_count}_op{args.output_len}_ep{args.epoch}"
    ...
    metric = evaluate.load("./metrics/rouge", local_files_only=True)
    ...
    def compute_metrics_rougel(eval_preds):
        ...
        targets = np.where(targets < 0, pad_id, targets)
        preds = preds.tolist()
    ...
    if trainer.is_world_process_zero():
        ...
        scores = get_scores(results_ans, results_rationale, results_reference, os.path.join(args.data_root, "data/problems.json"))
        preds = [pred.strip() for pred in preds]
        output_data = {
                "num_fail": num_fail,
                "scores": scores,
...
if __name__ == '__main__':
    ...
    print(f"\n===== Total running time: {elapsed/60:.2f} minutes ({elapsed:.2f} seconds) =====")
